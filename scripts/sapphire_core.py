...
Usage:
    from sapphire_core import (
        ...
        compute_auc, compute_marker_corr, shuffle_time_null,
        plot_dataset
    )
"""
...
# ============================================================
# Configuration (global, shared across all pipelines)
# ============================================================

import os

# DATA_ROOT can be overridden with an environment variable, e.g.:
#   export SAPPHIRE_DATA_ROOT=/path/to/your/data
# Defaults to a "data/" folder next to this script if not set.
...
DATASETS_CONFIG = {
    "Cardiomyocyte": {
        "file"         : DATA_ROOT / "E-MTAB-6268_allTP_merged_FIXED.h5ad",
        "time_col"     : "timepoint",
        "early_tp"     : "D0",
        "late_tp"      : "D30",
        "already_log1p": False,
    ...
    "Endoderm": {
        "file"         : DATA_ROOT / "GSE75748_Endoderm_merged_qc_umap_FIXED.h5ad",
        "time_col"     : "timepoint",
        "early_tp"     : "00h",
        "late_tp"      : "96h",
        "already_log1p": False,
    ...
    "Kidney": {
        "file"         : DATA_ROOT / "GSE118184_Takasato_iPS_timecourse_FIXED.h5ad",
        "time_col"     : "timepoint",
        "early_tp"     : "Day7",
        "late_tp"      : "Day26",
        "already_log1p": False,
...
def load_and_prepare(name, cfg, max_cells=None):
    """Read, subsample, normalize. max_cells defaults to the global MAX_CELLS."""
    if max_cells is None:
        max_cells = MAX_CELLS

    print(f"  Reading {cfg['file'].name}...")
    adata    = sc.read_h5ad(cfg["file"])
    time_col = cfg["time_col"]

    # Filter out Unknown timepoint
    if "Unknown" in adata.obs[time_col].values:
        n_before = adata.n_obs
        adata = adata[adata.obs[time_col] != "Unknown"].copy()
        print(f"  Filtered Unknown: {n_before:,} -> {adata.n_obs:,} cells")
...
def hvg_filter(adata, n_top=2000):
    """Select top N genes by variance, return filtered AnnData."""
...
def build_network(adata, params):
    """
    Label-free Spearman correlation network + greedy modularity community detection.
    Returns modules dict and gene list.
    """
...
def compute_per_cell_metrics(adata, modules, time_col):
    """Compute pathway_entropy and network_dispersion (both fully per-cell, label-free)."""
    X = adata.X
    if sp.issparse(X):
        X = X.toarray()
    ...
    # Network dispersion: per-cell, label-free kNN density dispersion.
    # FIX: the previous version computed a single median cosine distance to the
    # per-timepoint centroid and assigned that SAME value to every cell within a
    # timepoint. With only as many distinct values as there are timepoints, AUC
    # on early-vs-late collapses to a trivial 0/0.5/1.0 (this is exactly the
    # "earlier design" the paper's Methods 3.5 says was replaced). This block now
    # matches that Methods description and labelfree_dispersion.py: for each cell,
    # mean cosine distance to its k=30 nearest neighbours in module-activation
    # space, computed without using timepoint labels at any stage.
    from sklearn.preprocessing import normalize as _l2_normalize
    from sklearn.neighbors import NearestNeighbors

    K_DISPERSION = 30
    A_norm = _l2_normalize(A, norm="l2")
    k_eff  = min(K_DISPERSION + 1, n_cells)
    nn = NearestNeighbors(n_neighbors=k_eff, metric="cosine", algorithm="brute")
    nn.fit(A_norm)
    distances, _ = nn.kneighbors(A_norm)   # shape (n_cells, k_eff); col 0 = self (distance 0)
    dispersion = distances[:, 1:].mean(axis=1)
...
def compute_composite(pc_df):
    """Rank-based composite = (rank_entropy + rank_dispersion) / 2."""
    n      = len(pc_df)
...
def compute_auc(pc_df, early_tp, late_tp, metric):
    """AUC for early vs late separation."""
...
def compute_marker_corr(adata, pc_df, stem_markers, diff_markers):
    """Spearman correlation between pathway_entropy and marker gene scores."""
    results = {}
    ...
    for label, markers in [("stem", stem_markers), ("diff", diff_markers)]:
        ...
        print(f"    entropy vs {label}: ρ={r:.3f}, p={p:.2e}  (genes: {available})")

    return results


def shuffle_time_null(adata, modules, time_col, n_iter=50, random_state=0):
    """Shuffle-time null test. Returns (real_sign_changes, null_list, p_value)."""
...
def plot_dataset(name, pc_df, cfg, out_dir):
    """Three-panel trajectory plot: entropy / dispersion / composite."""
    time_col = cfg["time_col"]
    tps = sorted(
